...
<<<<<<< HEAD
	...
	/* POLAR:Set readFromQueue default value to be false */
...
restart:
	...
	/* POLAR: This is not read from queue */
	state->noPayload = false;
...
<<<<<<< HEAD
	...
	if (POLAR_ENABLE_XLOG_BUFFER())
	{
		...
		/*
		 * POLAR: When master or standby in recovery, pread blocks will be set to polar_read_ahead_xlog_num,
		 * we need to remove extra polar_read_ahead_xlog_num buffers
		 */
		if (AmStartupProcess())
...
#define COPY_HEADER_FIELD(_dst, _size)			\
	 ...
	 * In order to reserve XLOG queue space to store record meta.
	 */
	state->polar_logindex_meta_size = Max(ptr - (char *)record, 0);
