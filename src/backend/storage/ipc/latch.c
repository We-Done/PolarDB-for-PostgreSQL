 ...
 * As far as we have to preserve positions of all other events,
 ...
 * This mapping is stored in "polar_permutation" array. Also we need backward mapping
 * (from event to descriptors array) which is implemented using "polar_index" field of WaitEvent.
 */
=======
...
struct WaitEventSet
{
	...
	int         polar_free_events;    /* L1-list of free events linked by "pos" and terminated by -1. */
	...
	int        *polar_permutation;    /* indexes of used events (see comment above) */
...
CreateWaitEventSet(ResourceOwner resowner, int nevents)
{
	...
	sz += MAXALIGN(sizeof(int) * nevents);/* for polar_permutation */
	...
	/* POLAR: Shared Server */
	set->polar_permutation = (int *) data;
...
>>>>>>> c1ff2d8bc5be55e302731a16aaff563b7f03ed7c
	{
		WaitEvent* cur_event = &set->events[set->polar_permutation[i]];
		...
		else
		{
			...
			WSACloseEvent(set->handles[cur_event->polar_index + 1]);
		}
	}
#endif
				  ...
				  void *user_data)
{
	...
	free_event = set->polar_free_events;
	if (free_event >= 0)
	{
		event = &set->events[free_event];
		set->polar_free_events = event->pos;
	...
	set->polar_permutation[set->nevents] = event->pos;
	event->polar_index = set->nevents++;
	event->fd = fd;
	event->events = events;
...
void DeleteWaitEventFromSet(WaitEventSet *set, int event_pos)
{
	...
	if (--set->nevents != 0)
	{
		set->polar_permutation[event->polar_index] = set->polar_permutation[set->nevents];
		set->events[set->polar_permutation[set->nevents]].polar_index = event->polar_index;
	}
	event->fd = PGINVALID_SOCKET;
	event->events = 0;
	event->polar_index = -1;
	event->pos = set->polar_free_events;
	set->polar_free_events = event_pos;
}
...
{
	struct pollfd *pollfd = &set->pollfds[event->polar_index];
...
{
	HANDLE	   *handle = &set->handles[event->polar_index + 1];
					  ...
					  WaitEvent *occurred_events, int nevents)
{
	...
	for (i = 0; i < set->nevents && returned_events < nevents; i++, cur_pollfd++)
	{
		...
		cur_event = &set->events[set->polar_permutation[i]];
		occurred_events->pos = cur_event->pos;
		occurred_events->user_data = cur_event->user_data;
		occurred_events->events = 0;
					  ...
					  WaitEvent *occurred_events, int nevents)
{
	 ...
	 */
	cur_event = (WaitEvent *) &set->events[set->polar_permutation[rc - WAIT_OBJECT_0 - 1]];
...
<<<<<<< HEAD
	...
	else if (cur_event->events & WL_SOCKET_MASK)
	{
		...
		HANDLE		handle = set->handles[cur_event->polar_index + 1];
